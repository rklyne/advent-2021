...
def part1():
    lengths = {n: 0 for n in range(10)}
    for sequences, outputs in INPUT:
        for x in outputs:
            lengths[len(x)] += 1
    return lengths[2] + lengths[3] + lengths[4] + lengths[7]

valid_values = DIGITS.values()

def doReplace(digit_string, mapping):
    result =  ''.join(sorted([mapping[char] for char in digit_string]))
    # print(f"replaces {digit_string} -> {result} with {mapping}")
    return result

def doMap(outputs, mapping):
    mapped_outputs = [
            DECODE[doReplace(digit_string, mapping)]
            for digit_string in outputs
    ]
    # print(f"replaces {outputs} -> {mapped_outputs} with {mapping}")
    return mapped_outputs

letters = 'abcdefg'
possible_mappings = list(map(lambda p: dict(zip(letters, p)), itertools.permutations(letters)))

def determineMap(seqs):
...
def part2():
    total = 0
    for sequences, outputs in INPUT:
        out = doMap(outputs, determineMap(sequences))
        display = int(''.join([str(digit) for digit in out]))
        total += display
    return total
